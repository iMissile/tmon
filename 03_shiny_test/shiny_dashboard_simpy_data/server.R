...
shinyServer(function(input, output, session) {
  ...
  observe({
    updateDateRangeInput(session, "DateRange",
      ...
      end = as.Date(max(time_series$timestamp)) + days(1),
      min = as.Date(min(time_series$timestamp)),
      max = as.Date(max(time_series$timestamp)) + days(1)
    )
  })
  ...
  observeEvent(input$corr_click, {
    ...
    vals$ts_choice1 <- as.character(vals$CorrMatClick$Var1)
    vals$ts_choice2 <- as.character(vals$CorrMatClick$Var2)
  })
  ...
  observeEvent(input$lagged_corr_click, {
    ...
    vals$l_ts_choice1 <- as.character(vals$laggedCorrMatClick$Var1)
    vals$l_ts_choice2 <- as.character(vals$laggedCorrMatClick$Var2)
  })
...
#     print(vals$ts_choice2)
  ...
  output$CCFPlot <- renderPlot({
    l_ts_choice1 <- str_replace(vals$l_ts_choice1, "_lag_\\d*", "")
    l_ts_choice2 <- str_replace(vals$l_ts_choice2, "_lag_\\d*", "")
    ts1 <- resid %>% select(get(l_ts_choice1))
    ts2 <- resid %>% select(get(l_ts_choice2))
    currentLag <- optLags[l_ts_choice2] - optLags[l_ts_choice1]
